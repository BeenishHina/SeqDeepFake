...
class DeformableRefinementAttention(torch.nn.Module):
    def __init__(
        self,
        d_model,
        nhead,
        ...
        self.offset_predictor = torch.nn.Sequential(
            torch.nn.Linear(
                d_model,
                d_model
            ),
    ...
    def forward(
        ...
        # ---------------------------------
        # learn lightweight offsets
        # ---------------------------------
        offsets = self.offset_predictor(
            tgt
        )

        query = tgt + 0.1 * offsets

        # ---------------------------------
        # deformable refinement attention
        # ---------------------------------
